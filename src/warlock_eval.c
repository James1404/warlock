...
Sexp Eval_Quote(SexpAllocator* alloc, Sexp sexp) {
    if(ATOM_TY(alloc, sexp) != ATOM_CONS) return ATOM_MAKE(alloc, ATOM_NIL);
...
Sexp Eval_Cons(SexpAllocator* alloc, Sexp sexp) {
    ...
    while(ATOM_TY(alloc, current) != ATOM_NIL) {
    ...
    if(ATOM_TY(alloc, fn) == ATOM_FN) {
        ...
        Sexp result = ATOM_MAKE(alloc, ATOM_NIL);
        ...
        while(ATOM_TY(alloc, currentPassedArg) != ATOM_NIL &&
              ATOM_TY(alloc, currentFnArg) != ATOM_NIL) {
...
Sexp Eval_Lambda(SexpAllocator* alloc, Sexp sexp) {
    Sexp predicate = Sexp_First(alloc, sexp);
    if(!Eval_match(alloc, predicate, "lambda")) return ATOM_MAKE(alloc, ATOM_NIL);
